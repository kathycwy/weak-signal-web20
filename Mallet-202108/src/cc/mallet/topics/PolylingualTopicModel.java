...
public class PolylingualTopicModel implements Serializable {
     ...
     *
     * @param interval DataCollection.Save a copy of the state every <code>interval</code>
     *                 iterations.
     * @param filename DataCollection.Save the state to this file, with the iteration number as a
     *                 suffix
     */
    ...
    protected void sampleTopicsForOneDoc(final TopicAssignment topicAssignment, final boolean shouldSaveState) {
        ...
        for (int language = 0; language < numLanguages; language++) {
            ...
            for (int position = 0; position < docLength; position++) {
                ...
                // DataCollection.Save the smoothing-only mass to the global cache
